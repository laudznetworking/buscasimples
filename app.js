...
// ── Controle de Acesso Seguro (Supabase) ──────────────────────────────────────
let isUsuarioAdmin = false; 

async function verificarNivelAcesso(emailLogado) {
    try {
        ...
        if (data && data.email) {
            isUsuarioAdmin = true;
            console.log('[Security] Modo Admin Ativado.');
        } else {
...
// Mantemos o mesmo nome de função para não quebrar o resto do seu código
...
async function entrarNoSistema(email) {
    ...
    // NOVO: Pergunta ao banco de dados se este utilizador é admin
    await verificarNivelAcesso(email);

    // Exibe botões restritos a admins (O resto do seu código continua igual daqui para baixo)
...
// ─────────────────────────────────────────────────────────────────────────
// API Fortigate: Teste de MPLS via Backend Node.js
// ─────────────────────────────────────────────────────────────────────────
const BACKEND_URL = 'http://172.16.113.25:3000/api/teste-mpls';

async function testarMplsAPI(ipMpls) {
    if (!ipMpls || ipMpls === '---') return;

    // Abre a janela do terminal e limpa o conteúdo anterior
    abrirTerminal(ipMpls);
    const tela = document.getElementById('telaTerminal');
    if (tela) tela.innerHTML = '';
    
    adicionarLinhaTerminal(`[Sistema] Iniciando ponte SSH com Fortigate Concentrador...`, 'text-purple-400 font-bold');
    adicionarLinhaTerminal(`[Sistema] Alvo MPLS: ${ipMpls}`, 'text-slate-400');
    adicionarLinhaTerminal(`Aguardando resposta do FortiOS... (isto pode demorar 5 a 10 segundos)`, 'text-yellow-400 animate-pulse');

    try {
        // Faz a requisição ao servidor Node.js em background
        const resposta = await fetch(BACKEND_URL, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ ipMpls: ipMpls })
        });

        const dados = await resposta.json();

        if (tela) tela.innerHTML = '';
        adicionarLinhaTerminal(`root@fortigate-concentrador:~# execute ping ${ipMpls}`, 'text-emerald-500 mb-2');

        if (dados.sucesso) {
            // Imprime o retorno do Fortigate linha por linha
            const linhas = dados.resultado_bruto.split('\n');
            linhas.forEach(l => {
                if(l.trim() !== '') adicionarLinhaTerminal(l, 'text-slate-300 font-ip');
            });
            adicionarLinhaTerminal(`[Sistema] Teste SSH concluído. Conexão encerrada.`, 'text-purple-400 mt-4 font-bold');
        } else {
            adicionarLinhaTerminal(`[Erro Fortigate] ${dados.erro}`, 'text-red-500 font-bold');
        }

    } catch (erro) {
        if (tela) tela.innerHTML = '';
        adicionarLinhaTerminal(`[Falha Crítica] Não foi possível conectar ao Backend Node.js.`, 'text-red-500 font-bold text-lg');
        adicionarLinhaTerminal(`Verifique se o servidor (${BACKEND_URL}) está ligado e com a porta 3000 aberta no Firewall.`, 'text-slate-400 mt-2');
        console.error('Erro na API SSH:', erro);
    }
}
// ══════════════════════════════════════════════════════════════════════════════
...
// Quando for montar o cartão:
const nomeSeguro = escaparHTML(cliente.nome);
            ...
            <div class="space-y-4">
                ...
                <div class="bg-gradient-to-r from-black/60 to-black/30 p-4 rounded border border-white/5
                    ...
                    <div class="flex gap-2">
                        <button onclick="testarMplsAPI('${c.ip_mpls}')" title="Testar Ping MPLS Automático" class="btn-modern bg-purple-500/10 hover:bg-purple-500 text-purple-400 hover:text-white p-3 rounded-sm border border-purple-500/20 transition-all shadow-lg">
                            <i class="fas fa-satellite-dish animate-pulse"></i>
                        </button>
                        
                        <button onclick="copiarComandoMpls('${c.ip_mpls}', this)" title="Copiar comandos para Fortigate" class="btn-modern bg-emerald-500/10 hover:bg-emerald-500 text-emerald-400 hover:text-white p-3 rounded-sm border border-emerald-500/20 transition-all shadow-lg">
                            <i class="fas fa-terminal"></i>
                        </button>
                        
                        <button onclick="copiarTexto('${c.ip_mpls}', this)" title="Copiar apenas o IP" class="btn-modern bg-white/5 hover:bg-white/10 text-slate-400 hover:text-white p-3 rounded-sm border border-white/5 transition-all">
